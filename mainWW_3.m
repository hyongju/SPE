...
for i = 1:size(timeSeriesUniFixed,2)
%     k = k+1;
    posExt{i} = [(timeSeriesUniFixed{i}(:,4)-bndPnts0(1))/(bndPnts0(2)-bndPnts0(1)) ...
        (timeSeriesUniFixed{i}(:,3)-bndPnts0(3))/(bndPnts0(4)-bndPnts0(3))];
    obserV{i} = timeSeriesUniFixed{i}(:,6);
    for j = 1:size(obserV{i},1)
        if obserV{i}(j) == 3
           obserV{i}(j) = 1.5;
        elseif obserV{i}(j) == 4
            obserV{i}(j) = 0;
...
for i = 1:size(b_Poly,1)
    b_Poly{i}(:,1) = (b_Poly{i}(:,1)-bndPnts0(1))/(bndPnts0(2)-bndPnts0(1));
    b_Poly{i}(:,2) = (b_Poly{i}(:,2)-bndPnts0(3))/(bndPnts0(4)-bndPnts0(3));
end

%% 
rMax = 0.1; % 1 mile
nRuns = size(timeSeriesUniFixed,2)+1;  % total number of runs
n = 10;         % number of robots
orderK = 1;    % order k, available options, 1, 2, n
...
% save samples...
particleWgt = ones(1,size(sampPos,1))*0.3;
...
%     nRuns
    ...
    disp([num2str(curStep) ' out of ' num2str(nRuns)]);
...
%                 gTruth(i,:) =  obs(idxl(1))/2;
    ...
    % findMinDist
    markSamp = zeros(size(sampPos,1),1);
    for i = 1:size(sampPos,1)
        for j = 1:length(sampInfo)
            if norm(sampPos(i,:)-pos(friend{i}(1),:)) < rMax  
...
%                     markSamp(i) = 1;
    ...
    for i = 1:size(sampPos,1)
        if markSamp(i) ~= 1
            ...
            wgt2Avg(i) = sampInfo' * wgt2(:,i) / size(sampPos,1);
        end
    end

    particleWgt = wgt2Avg/sum(wgt2Avg);
    if rem(curStep,100) ==1
