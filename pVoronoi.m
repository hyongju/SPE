function [vornb,vorvx,Aaug,baug] = pVoronoi(pos,bnd_pnts)
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------
% [Voronoi neighbor,Voronoi vertices] = pVoronoi(points, boundary)
...
for i = 1:size(pos,1)
    ...
    for j = 1:size(vornb{i},2)
        k = k + 1;
        [A{i}(k,:),b{i}(k,:)] = perpBisec(pos(i,:), pos(vornb{i}(j),:));
    end
end
