...
% numKernels = 10;
rng('shuffle')
% numPeaks = randsample(1:numKernels,1);
numPeaks = 5;
posPeaks = rand(numPeaks,2);
gTruthTmp = [];
for i = 1:size(posPeaks,1)
    mix{i} = mvnpdf(sampPos,posPeaks(i,:),0.05*eye(2));
    if i == 1
        gTruthTmp = mix{i};
    else
        gTruthTmp = gTruthTmp + mix{i};
    end
end
...
for curStep = 2:nRuns
    ...
    for i = 1:size(sampPos,1)
        ...
        idxl = findMinDistBnd(sampPos(i,:),pos,orderK,effRad);
        if isempty(idxl)
            detectLhd(i) = 0;
        else
            switch(orderK)
